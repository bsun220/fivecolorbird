...
class AdminLists extends BaseAdminDataLists implements ListsExtendInterface, ListsSearchInterface, ListsSortInterface,ListsExcelInterface
{
    ...
    public function queryWhere()
    {
        ...
        if (isset($this->params['dept_id']) && $this->params['dept_id'] != '') {
            $adminIds = AdminDept::where('dept_id', $this->params['dept_id'])->column('admin_id');
            if (!empty($adminIds)) {
                $where[] = ['id', 'in', $adminIds];
            }else{
                $where[] = ['id', 'in', [0]];
        ...
        if (isset($this->params['jobs_id']) && $this->params['jobs_id'] != '') {
            $adminIds = AdminJobs::where('jobs_id', $this->params['jobs_id'])->column('admin_id');
            if (!empty($adminIds)) {
                $where[] = ['id', 'in', $adminIds];
            }else{
                $where[] = ['id', 'in', [0]];
    ...
    public function lists(): array
    {
        $field = [
            ...
            'login_time', 'login_ip', 'multipoint_login', 'avatar', 'age', 'email', 'card_val', 'salary', 'address', 'entry_time', 'sex', 'card', 'qualification', 'mobile', 'number', 'contract_file', 'password_mw'
        ];
